...
def build_EIANN_from_config(config_path, network_seed=42, config_format='normal', training_kwargs=None, device='cpu'):
    '''
    ...
    try:
        network = nt.Network(layer_config, projection_config, seed=network_seed, device=device, **training_kwargs)
    except:
        projection_config = convert_projection_config_dict(projection_config)
        layer_config = convert_layer_config_dict(layer_config)
        network = nt.Network(layer_config, projection_config, seed=network_seed, device=device, **training_kwargs)
