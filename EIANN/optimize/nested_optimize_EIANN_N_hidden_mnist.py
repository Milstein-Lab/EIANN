...
# Layer-count-agnostic MNIST driver. Reads hidden-layer names from layer_config
# (any keys that are not 'Input' or 'Output') so the same script works for 1, 2,
# 3, ... hidden layers. Intended for Ray vs MPI optimization benchmarking on
# wider/deeper networks.
#
# Example (serial, 2-hidden van_bp, 1 seed, 1 epoch):
# python -m nested.analyze --framework=serial \
#   --config-file-path=optimize/optimize_config/mnist/<new 2-hidden large>.yaml \
#   --param-file-path=optimize/optimize_params/mnist/20240816_nested_optimize_2_hidden_mnist_params.yaml \
#   --model-key=van_bp_relu --output-dir=optimize/data --num_instances=1 --epochs=1 --status_bar=True
...
def config_worker():
    ...
    if not context.retrain:
        if 'data_file_path' not in context() or not os.path.exists(context.data_file_path):
            raise Exception('nested_optimize_EIANN_N_hidden_mnist: missing valid data_file_path to load network from '
                            'file')
    ...
    # Discover hidden layers: any layer_config key that isn't Input or Output.
    # Order is preserved from YAML (Python 3.7+ dicts).
    context.hidden_layers = [name for name in context.layer_config
    ...
    if requested_device not in ('cpu', 'cuda', 'auto'):
        raise Exception('nested_optimize_EIANN_N_hidden_mnist: device must be one of cpu|cuda|auto, not %s' %
                        requested_device)
    if requested_device == 'auto':
        resolved_device = 'cuda' if torch.cuda.is_available() else 'cpu'
    elif requested_device == 'cuda' and not torch.cuda.is_available():
        raise Exception('nested_optimize_EIANN_N_hidden_mnist: device was set to cuda, but cuda is not available')
    else:
        resolved_device = requested_device
    context.training_kwargs['device'] = resolved_device
    if (('disp' in context() and context.disp) or context.debug):
        print('nested_optimize_EIANN_N_hidden_mnist: pid: %i hidden_layers=%s requested_device=%s resolved_device=%s '
              'cuda_available=%s cuda_device_count=%i CUDA_VISIBLE_DEVICES=%s' %
              (os.getpid(), context.hidden_layers, requested_device, resolved_device, torch.cuda.is_available(),
...
def _get_adjacent_layer_pairs():
    """Ordered list of (prev, next) layer-name pairs along the feedforward path.
    For hidden_layers=['H1','H2']: [('Input','H1'), ('H1','H2'), ('H2','Output')].
    """
    chain = ['Input'] + list(context.hidden_layers) + ['Output']
...
def compute_features(x, seed, data_seed, model_id=None, export=False, plot=False):
    ...
    if export:
        ...
        if context.disp:
            print('nested_optimize_EIANN_N_hidden_mnist: pid: %i exported network config to %s' %
                  (os.getpid(), context.export_network_config_file_path))

    if plot:
        # Snapshot initial forward/feedback weights for every FF pair; wrapped
        # because feedback projections only exist for some learning rules.
        for prev, nxt in _get_adjacent_layer_pairs():
            try:
                getattr(getattr(getattr(getattr(network, nxt), 'E'), prev), 'E').initial_weight = \
                    getattr(getattr(getattr(getattr(network, nxt), 'E'), prev), 'E').weight.data.detach().clone()
            except Exception:
                pass
            try:
                getattr(getattr(getattr(getattr(network, prev), 'E'), nxt), 'E').initial_weight = \
                    getattr(getattr(getattr(getattr(network, prev), 'E'), nxt), 'E').weight.data.detach().clone()
            except Exception:
                pass
    ...
    if not context.retrain:
        ...
        if context.disp:
            print('nested_optimize_EIANN_N_hidden_mnist: pid: %i loaded network history from %s' %
    ...
    if plot:
        try:
            ...
            for prev, nxt in _get_adjacent_layer_pairs():
                try:
                    ff = getattr(getattr(getattr(getattr(network, nxt), 'E'), prev), 'E')
                    fb = getattr(getattr(getattr(getattr(network, prev), 'E'), nxt), 'E')
                    plot_FB_weight_alignment(ff, fb)
                except Exception:
                    pass
        except Exception:
            pass
    ...
    if not context.supervised:
        if context.eval_accuracy == 'final':
            ...
            raise Exception('nested_optimize_EIANN_N_hidden_mnist: eval_accuracy must be final or best, not %s' %
    ...
    if plot:
        # Receptive fields and representation metrics for each hidden layer.
        for h_name in context.hidden_layers:
            if h_name not in network.layers:
                continue
            population = getattr(getattr(network, h_name), 'E')
            if context.compute_receptive_fields:
                ...
                receptive_fields = population.Input.E.weight.detach() if hasattr(population, 'Input') else None
                if receptive_fields is None:
                    # For deeper hidden layers there is no direct Input projection;
                    # fall back to maxact receptive fields.
                    receptive_fields = utils.compute_maxact_receptive_fields(
                        population, test_dataloader=test_dataloader)
            plot_receptive_fields(receptive_fields, sort=True, num_cols=10, num_rows=10)

            if context.full_analysis:
                metrics_dict = utils.compute_representation_metrics(population, test_dataloader, receptive_fields)
    ...
    if export:
        ...
        if context.disp:
            print('nested_optimize_EIANN_N_hidden_mnist: pid: %i exported network history to %s' %
