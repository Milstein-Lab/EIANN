...
srun --overlap --nodes=1 --ntasks=1 -w "$head_node" python -m nested.optimize --config-file-path=$1 \
  ...
  --pop_size=9 --max_iter=2 --path_length=2 --num_cpus=1 --num_gpus=0.5 --device=$DEVICE --val_batch_size=1000
...
# sbatch optimize_EIANN_ray_multinode_frontera_cifar10.sh optimize/optimize_config/cifar10/20250814_nested_optimize_EIANN_2_hidden_convnet_cifar10_van_bp_relu_SGD_CE_config_G_learned_bias.yaml cuda
