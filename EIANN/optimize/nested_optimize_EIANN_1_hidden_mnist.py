...
def config_worker():
    ...
    if requested_device not in ('cpu', 'cuda', 'auto'):
        raise Exception('nested_optimize_EIANN_1_hidden_mnist: device must be one of cpu|cuda|auto, not %s' %
                        requested_device)
    if requested_device == 'auto':
        resolved_device = 'cuda' if torch.cuda.is_available() else 'cpu'
    elif requested_device == 'cuda' and not torch.cuda.is_available():
        raise Exception('nested_optimize_EIANN_1_hidden_mnist: device was set to cuda, but cuda is not available')
    else:
        resolved_device = requested_device
    context.training_kwargs['device'] = resolved_device
    if (('disp' in context() and context.disp) or context.debug):
        print('nested_optimize_EIANN_1_hidden_mnist: pid: %i requested_device=%s resolved_device=%s '
