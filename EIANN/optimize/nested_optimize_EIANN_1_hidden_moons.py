...
def config_worker():
    ...
    x_train, labels_train = make_moons(256, noise=0.1, random_state=0)
    x_train = torch.tensor(x_train).float()
    x_train[:, 0] += 1.5
    x_train[:, 1] += 0.75
    data_train = []
    for idx, (sample, label) in enumerate(zip(x_train, labels_train)):
        target = torch.eye(output_size)[label]
        data_train.append((idx, sample, target))

    x_test, labels_test = make_moons(256, noise=0.1, random_state=1)
    x_test = torch.tensor(x_test).float()
    x_test[:, 0] += 1.5
    x_test[:, 1] += 0.75
    data_test = []
    for idx, (sample, label) in enumerate(zip(x_test, labels_test)):
        target = torch.eye(output_size)[label]
        data_test.append((idx, sample, target))

    x_val, labels_val = make_moons(256, noise=0.1, random_state=2)
    x_val = torch.tensor(x_val).float()
    x_val[:, 0] += 1.5
    x_val[:, 1] += 0.75
    data_val = []
    for idx, (sample, label) in enumerate(zip(x_val, labels_val)):
        target = torch.eye(output_size)[label]
        data_val.append((idx, sample, target))

    # Put data in dataloader
    context.data_generator = torch.Generator()
    context.train_sub_dataloader = \
        torch.utils.data.DataLoader(data_train, shuffle=True, generator=context.data_generator)
    context.val_dataloader = torch.utils.data.DataLoader(data_val, batch_size=256, shuffle=False)
    context.test_dataloader = torch.utils.data.DataLoader(data_test, batch_size=256, shuffle=False)
