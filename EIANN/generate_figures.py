...
def generate_Figure1(all_dataloaders, show=True, save=False):
    ...
    for i, model_name in enumerate(model_dict):
        ...
        pt.plot_batch_accuracy(network, test_dataloader, population=network.Output.E, ax=ax, export=True)
        ...
        num_units = 6
        rf_axes = gs.GridSpecFromSubplotSpec(2, 3, subplot_spec=axes[1, i], wspace=0.1, hspace=-0.4)

        ax_list = []
        for j in range(num_units):
            ax = fig.add_subplot(rf_axes[j])
            ax_list.append(ax)
        receptive_fields = ut.compute_maxact_receptive_fields(population, test_dataloader, export=True, num_units=num_units)
        im = pt.plot_receptive_fields(receptive_fields, num_units=10, ax_list=ax_list)
