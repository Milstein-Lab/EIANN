...
class BP_like_1E(LearningRule):
    def __init__(self, projection, max_pop_fraction=0.025, stochastic=True, learning_rate=None):
        ...
        Weight updates are proportional to local error and presynaptic firing rate.
        If unit selection is stochastic, hidden units are selected for a weight update with a probability proportional
        to dendritic state by sampling a Bernoulli distribution. Otherwise, units are sorted be dendritic state. A fixed
...
class almost_backprop1(LearningRule):
    ...
    def backward(cls, network, output, target, store_history=False, store_dynamics=False):
        ...
        for i,layer in enumerate(reversed_layers[:-1]): # Iterate over populations in reverse order starting from the output layer
            ...
            layer.E.dendritic_state = layer.E.dendritic_state * d_activation
            pre_layer.E.dendritic_state = forward_weight_transpose @ layer.E.dendritic_state
